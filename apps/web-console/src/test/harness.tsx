...
import type { Device, Environment, HealthCondition, HealthView, Session, Zone } from '../api/types';
...
export function stubFetch(handlers: Record<string, StubHandler>) {
  ...
  const mock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    calls.push({ key, init });
    return handlers[key]?.(init) ?? json({ error: 'unstubbed' }, 404);
  });
 ...
 * mutation test exercises the real `canMutate` gate instead of bypassing it.
 */
export function SignedIn({ children }: { children: ReactNode }) {
  ...
  // Gate on the completed sign-in, not on `canMutate`, so a later expiry keeps
