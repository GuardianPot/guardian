...
export function EnvironmentPage() {
  ...
  async function submitRename(event: FormEvent<HTMLFormElement>) {
    ...
    try { await rename.mutateAsync(String(new FormData(form).get('display_name') ?? '')); setMessage('Environment name updated.'); }
  ...
  async function submitZone(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await createZone.mutateAsync({ display_name: String(data.get('display_name') ?? ''), cidr: String(data.get('cidr') ?? '') });
  ...
  async function submitEnrollment(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const created = await api.createEnrollmentSecret(environmentId, String(new FormData(form).get('device_name') ?? ''), auth.csrf);
    ...
    <div>
      ...
      <div className={styles.twoColumn}>
        ...
        <section className={styles.panel} aria-labelledby="enroll-heading">
          ...
          <form className={styles.form} onSubmit={submitEnrollment}>
            <label>Device name<input name="device_name" required maxLength={128} disabled={!auth.canMutate || creatingSecret} /></label>
            <button className={styles.primaryButton} disabled={!auth.canMutate || creatingSecret}>{creatingSecret ? 'Creating…' : 'Create one-time secret'}</button>
          </form>
        </section>
      </div>
      ...
      <div className={styles.twoColumn}>
        <section className={styles.panel} aria-labelledby="zones-heading">
          ...
          <form className={styles.inlineForm} onSubmit={submitZone}>
            <label>Zone name<input name="display_name" required maxLength={128} disabled={!auth.canMutate} /></label>
            <label>Private CIDR<input name="cidr" placeholder="10.20.0.0/24" required maxLength={18} disabled={!auth.canMutate} /></label>
            <button className={styles.secondaryButton} disabled={!auth.canMutate || createZone.isPending}>Add zone</button>
          </form>
        </section>
        <section className={styles.panel} aria-labelledby="settings-heading">
          ...
          <form className={styles.form} onSubmit={submitRename}>
            <label>Display name<input name="display_name" defaultValue={environment.data.display_name} required maxLength={128} disabled={!auth.canMutate} /></label>
            <button className={styles.secondaryButton} disabled={!auth.canMutate || rename.isPending}>Save name</button>
          </form>
        </section>
      </div>
