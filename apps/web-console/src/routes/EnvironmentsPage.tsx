...
export function EnvironmentsPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await create.mutateAsync(String(new FormData(form).get('display_name') ?? ''));
      form.reset();
    } catch {
      ...
      <div className={styles.twoColumn}>
        ...
        <section className={styles.panel} aria-labelledby="create-environment-heading">
          ...
          <form className={styles.form} onSubmit={submit}>
            <label>Display name<input name="display_name" required maxLength={128} disabled={!auth.canMutate || create.isPending} /></label>
            {error && <p className={styles.formError} role="alert">{error}</p>}
            <button className={styles.primaryButton} disabled={!auth.canMutate || create.isPending}>
