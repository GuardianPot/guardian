...
describe('LoginPage', () => {
  it('sends credentials with one selected MFA proof and persists no secrets', async () => {
    const fetchMock = vi.fn<(input: RequestInfo | URL, init?: RequestInit) => Promise<Response>>(async (input) => {
      if (String(input) === '/v1/auth/session') return new Response('{}', { status: 401 });
    ...
    expect(JSON.parse(String(loginCall[1]?.body))).toEqual({ username: 'owner', password: 'correct horse battery staple', totp_code: '123456' });
  ...
  it('renders a generic denied state without reflecting submitted secrets', async () => {
    const fetchMock = vi.fn<(input: RequestInfo | URL) => Promise<Response>>(async (input) =>
      new Response('{}', { status: String(input) === '/v1/auth/session' ? 401 : 401 }),
    );
    vi.stubGlobal('fetch', fetchMock);
    const queryClient = new QueryClient({ defaultOptions: { queries: { retry: false } } });
    render(
