...
// Field is the closed set of environment and zone request-body field paths a
// rejection may name. It is a vocabulary rather than a free string because a
// field path travels to an operator's browser: change proposal 0004 requires
// that it name a field and never echo a submitted value.
type Field string
...
// a closed field path and a closed reason and has nowhere to put the submitted
// value, which is the point: echoing a rejected display name or CIDR back into
// an error response is the leakage path change proposal 0004 closes.
//
...
// without one names no request-body field — a stale revision or a missing
// environment — and callers must not invent a field for it.
...
// Fields returns every field path in declaration order as a defensive copy.
func Fields() []Field { return []Field{FieldDisplayName, FieldCIDR} }

// Reasons returns every reason in declaration order as a defensive copy.
func Reasons() []Reason { return []Reason{ReasonMalformed, ReasonOutOfRange, ReasonConflicting} }
